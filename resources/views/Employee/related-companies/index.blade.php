                                            ...
                                            <tbody>
                                                @foreach(im('employee')->contracts as $index=>$element)
                                                    <tr>
                                                        ...
                                                        <td>{!! $element->job->description !!}</td>
                                                        <td>{{ $element->job->work_days_in_week }}</td>
                                                        <td>{{ date($element->employee->pivotCompanyColumns($element->job->company->id)->salary_appointment_at) }}</td>
                                                        <td>{{ $element->employee->pivotCompanyColumns($element->job->company->id)->started_at }}</td>
                                                        <td>
                                                            <div class="btn-sm">
                                                                <a href="{{ route('employee.jobs.show',$element->id) }}" class="btn btn-success" style="margin-bottom: 5px">فسخ التعاقد</a>
